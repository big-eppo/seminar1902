...
gehorcht dem Prinzip der Privatautonomie im Sinne eigenverantwortlicher Handlungsfreiheit.
\grqq~\cite{ek}
\end{quote}
...
Sicherheit zum Ausgleich gelangt.  
\grqq~\cite{ek}
\end{quote}
...
Lage, in der ein automatisiertes Fahrzeug vor der ?Entscheidung? steht, eines von zwei
...
einer ?intelligenten? Straßen-Infrastruktur ? genutzt und kontinuierlich weiterentwickelt
werden. Die erhebliche Steigerung der Verkehrssicherheit ist Entwicklungs- und Regulierungsziel, und zwar bereits in der 
Auslegung und Programmierung der Fahrzeuge zu defensivem und vorausschauendem, schwächere Verkehrsteilnehmer (?Vulnerable Road
Users?) schonendem Fahren. 
\grqq~\cite{ek}
\end{quote}
...
ist (Verbot der Degradierung des Subjekts zum bloßen Netzwerkelement). 
\grqq~\cite{ek}
\end{quote}
...
so anzulegen, im Konflikt Tier- oder Sachschäden in Kauf zu nehmen, wenn dadurch Personenschäden vermeidbar sind. 
\grqq~\cite{ek}
\end{quote}
...
der konkreten tatsächlichen Situation unter Einschluss ?unberechenbarer? Verhaltensweisen Betroffener abhängig. 
...
eines Bundesamtes für Sicherheit im automatisierten und vernetzten Verkehr) Erfahrungen systematisch zu verarbeiten. 
\grqq~\cite{ek}
\end{quote}
...
Mobilitätsrisiken Beteiligten dürfen Unbeteiligte nicht opfern. 
\grqq~\cite{ek}
\end{quote}
...
Entscheidungspraxis müssen diesem Übergang hinreichend Rechnung tragen. 
\grqq~\cite{ek}
\end{quote}
...
verbessern, wo dies technisch möglich und zumutbar ist. 
\grqq~\cite{ek}
\end{quote}
...
kommuniziert und von einer fachlich geeigneten, unabhängigen Stelle geprüft werden. 
\grqq~\cite{ek}
\end{quote}
...
und der Manipulation der Fahrzeugsteuerung nicht sicher auszuschließen vermag.  
\grqq~\cite{ek}
\end{quote}
...
sollte frühzeitig entgegengewirkt werden. 
\grqq~\cite{ek}
\end{quote}
...
mit der Möglichkeit des ?Overrulings? Verantwortung behält. Bei nicht fahrerlosen Systemen muss die Mensch/Maschine-Schnittstelle 
...
Dokumentationspflichten zu gewährleisten. 
\grqq~\cite{ek}
\end{quote}
...
die Notwendigkeit einer abrupten Übergabe der Kontrolle an den Fahrer (?Notstand?)
...
und nicht umgekehrt erhöhte Anpassungsleistungen dem Menschen abverlangt werden.  
\grqq~\cite{ek}
\end{quote}
...
entsprechende allgemeingültige Vorgaben, einschließlich etwaiger Abnahmetests zu erstellen. 
\grqq~\cite{ek}
\end{quote}
...
einen ?sicheren Zustand? gelangen. Eine Vereinheitlichung insbesondere der Definition
des sicheren Zustandes oder auch der Übergaberoutinen ist wünschenswert. 
\grqq~\cite{ek}
\end{quote}
...
bei der Fahrausbildung in geeigneter Weise vermittelt und geprüft werden. 
\grqq~\cite{ek}
\end{quote}
